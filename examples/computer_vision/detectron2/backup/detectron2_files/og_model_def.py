...
from detectron2_files.cocoeval import *
    

import detectron2.utils.comm as comm
TorchData = Union[Dict[str, torch.Tensor], Sequence[torch.Tensor], torch.Tensor]

class OGDetectronTrial(PyTorchTrial):
    ...
    def build_validation_data_loader(self) -> DataLoader:
        print ('building validation data_loader')
...
def get_evaluator(cfg, dataset_name, output_folder=None):
    ...
    if evaluator_type in ["coco", "coco_panoptic_seg"]:
        t = COCOEvaluator(dataset_name, cfg, False)
        print ('in coco: ', t, len(evaluator_list))
        evaluator_list.append(t)
        print ('in coco after: ', t, len(evaluator_list))
