...
class PyTorchTrialController(det.LoopTrialController):
    ...
    def _train_for_step(
        ...
        # XXX: wait, it's not really right to apply the reducers to the the per_batch_metrics
        # (arguably we should be adding a dimension to the per_batch_metrics to represent the
        # multiple workers, because doing a custom reduction each individual value seems insane),
        # so we are just going to skirt that issue for now.
